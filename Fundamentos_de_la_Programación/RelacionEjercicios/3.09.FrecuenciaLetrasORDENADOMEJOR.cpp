...
const int N = 26;
typedef array<int, N> TFrec;        ///PORQUE HAY 26 LETRAS MAYUSCULAS

void calcular(TFrec& frecuencia)
{
    char c;
    ...
    while (c != '.')
    {
        if (c >= 'A' && c <= 'Z')
        {
            frecuencia[c - 'A']++;      // c-A calcula la posicion de la letra que por su naturaleza ocupara una de las 26 letras posibles posiciones des 0 a 25
        }
        cin.get(c);
    }
}

void imprimir(TFrec& frecuencia)
{
    for (int i = 0;i < N;i++ )
    {
        if (frecuencia[i] != 0)
...
int main(){
    TFrec frecuencia = {{}};    /// INICIALIZA EN 0
