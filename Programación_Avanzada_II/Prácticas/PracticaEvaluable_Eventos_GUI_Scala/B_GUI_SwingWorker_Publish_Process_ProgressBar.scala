...
import A_GUI_con_SwingWorker._


class Panel_B extends JPanel { // Panel hereda de JPanel => esta clase será una ventana/panel personalizado para la GUI
  /**
   * // Patrón seguido
  ...
  // Barras de progreso para el cálculo de estos primos(Apartado B)
  ...
  private def init(): Unit = {
    ...
    // Configuración de las barras de progreso(Apartado B)
    // Inicializa el valor de la barra y habilita mostrar texto en la barra(%)
  ...
  def nuevoProgreso(n: Int, tipo: String): Unit = {

    if (tipo == "twin") {
      progressTwin.setValue(n)
    } else if (tipo == "cousin") {
      progressCousin.setValue(n)
    } else {
      progressSexy.setValue(n)
    }

  }
}

class Controlador_B(panel: Panel_B) extends ActionListener, PropertyChangeListener {
  private var workerTwin: WorkerTwin_B = _
  private var workerCousin: WorkerCousin_B = _
  private var workerSexy: WorkerSexy_B = _

  private val twinPropiedadProgreso = new TwinPropiedadProgreso(panel)
  private val cousinPropiedadProgreso = new CousinPropiedadProgreso(panel)
  private val sexyPropiedadProgreso = new SexyPropiedadProgreso(panel)
  ...
  override def actionPerformed(e: ActionEvent): Unit = {
    ...
    if (comando.equals("CANCELAR")) {
      ...
      try {
        ...
        panel.nuevoProgreso(0, tipo)
        panel.nuevoMensaje("Calculando primos...", "")

        val workerGeneral = tipo match {
          case "twin" =>
            workerTwin = new WorkerTwin_B(n, panel)
            workerTwin.addPropertyChangeListener(twinPropiedadProgreso)
            workerTwin
          case "cousin" =>
            workerCousin = new WorkerCousin_B(n, panel)
            workerCousin.addPropertyChangeListener(cousinPropiedadProgreso)
            workerCousin
          case _ =>
            workerSexy = new WorkerSexy_B(n, panel)
            workerSexy.addPropertyChangeListener(sexyPropiedadProgreso)
            workerSexy
        }
  ...
  override def propertyChange(evt: PropertyChangeEvent): Unit = {
    // No la usamos al definirla en las 3 clases inferiores
  }

}

class TwinPropiedadProgreso(panel: Panel_B) extends PropertyChangeListener {
  override def propertyChange(evt: PropertyChangeEvent): Unit = {
    if (evt.getPropertyName == "progress") {
      panel.nuevoProgreso(evt.getNewValue.asInstanceOf[Int], "twin")
    }
  }
}

class CousinPropiedadProgreso(panel: Panel_B) extends PropertyChangeListener {
  override def propertyChange(evt: PropertyChangeEvent): Unit = {
    if (evt.getPropertyName == "progress") {
      panel.nuevoProgreso(evt.getNewValue.asInstanceOf[Int], "cousin")
    }
  }
}

class SexyPropiedadProgreso(panel: Panel_B) extends PropertyChangeListener {
  override def propertyChange(evt: PropertyChangeEvent): Unit = {
    if (evt.getPropertyName == "progress") {
      panel.nuevoProgreso(evt.getNewValue.asInstanceOf[Int], "sexy")
    }
  }
}

class WorkerTwin_B(n: Int, panel: Panel_B) extends SwingWorker[Unit, String] {
...
class WorkerCousin_B(n: Int, panel: Panel_B) extends SwingWorker[Unit, String] {
  def esPrimo(n: Int): Boolean = {
    @tailrec
    def loop(div: Int): Boolean = {
      if (div * div > n) true
...
class WorkerSexy_B(n: Int, panel: Panel_B) extends SwingWorker[Unit, String] {
...
object B_GUI_SwingWorker_Publish_Process_ProgressBar {
  // Función para crear la GUI
  def crearGUI(ventana:JFrame):Unit = {
    val panel = new Panel_B // Creamos el panel
    val controlador = new Controlador_B(panel)
